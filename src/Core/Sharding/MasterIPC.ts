...
import { IPCEvent, IPCError, IPCResult, IPCEvents, IPCEvalResults, SharderEvents } from "../../types";
import {SendOptions, Server, NodeMessage} from "veza";
...
function getIdFromSocketName(name: string | null): string | number | null {
    if (!name)
        return null;

    if (name.startsWith("Hyperion:Service:"))
...
export class MasterIPC {
	...
	constructor(public manager: ShardManager) {
	    ...
	    this.server = new Server("Hyperion:Master")
	...
	public static clusterRecipient(clusterId: number): string {
	    return "Hyperion:Cluster:" + clusterId;
	}

	public static serviceRecipient(serviceName: string): string {
	    return "Hyperion:Service:" + serviceName;
	}

	public sendTo(recipient: string, data: IPCEvent, options: SendOptions = { }): Promise<any> {
	    ...
	    return this.server.sendTo("Hyperion:" + recipient, data, options);
	}

	public broadcast(data: IPCEvent, options: SendOptions = { }): Promise<any[]> {
	    if (typeof data !== "object" || data.op === undefined)
	        throw new Error("Message data not an object, or no op code was specified");
	...
	public async sendEval(script: string | Function, clusterId: number): Promise<unknown[]> {
	...
	public async broadcastEval(script: string | Function): Promise<IPCEvalResults> {
	    script = typeof script === "function" ? `(${script})(this)` : script;
	...
	public async sendCommand(serviceName: string, data: any, options: SendOptions = { }): Promise<unknown[]> {
	...
	private ["_" + IPCEvents.READY](message: NodeMessage, data: any): void {
	    if (message.client.name!.startsWith("Hyperion:Cluster:")) {
	        const cluster = this.manager.clusters!.get(data.id);
			cluster!.emit("ready");

			this.manager.emit(SharderEvents.CLUSTER_READY, cluster);
	    } else if (message.client.name!.startsWith("Hyperion:Service:")) {
	...
	private ["_" + IPCEvents.SHARD_CONNECTED](message: NodeMessage, data: any): void {
	    this.manager.emit(SharderEvents.SHARD_CONNECTED, data.id, data.shardId);
	}

	private ["_" + IPCEvents.SHARD_READY](message: NodeMessage, data: any): void {
	    this.manager.emit(SharderEvents.SHARD_READY, data.id, data.shardId);
	}

	private ["_" + IPCEvents.SHARD_RESUMED](message: NodeMessage, data: any): void {
	    this.manager.emit(SharderEvents.SHARD_RESUMED, data.id, data.shardId);
	}

	private ["_" + IPCEvents.SHARD_DISCONNECTED](message: NodeMessage, data: any): void {
	    this.manager.emit(SharderEvents.SHARD_DISCONNECT, data.id, data.shardId, data.error);
	}

	private ["_" + IPCEvents.ERROR](message: NodeMessage, data: any): void {
	    this.manager.emit(SharderEvents.ERROR, data.error, data.id, data.shardId);
	}

	private async ["_" + IPCEvents.SHUTDOWN](message: NodeMessage, data: any): Promise<boolean | void> {
	    try {
	...
	private async ["_" + IPCEvents.SERVICE_EVAL](message: NodeMessage, data: any): Promise<void> {
	    try {
	...
	private ["_" + IPCEvents.GET](message: NodeMessage, data: any): void {
	...
	private ["_" + IPCEvents.SET](message: NodeMessage, data: any): void {
	    if (!data.key || typeof data.key !== "string")
	        return message.reply({ success: false, d: { name: "Error", message: "Key is a required string" } });
	    if (!data.value)
	...
	private async ["_" + IPCEvents.FETCH_USER](message: NodeMessage, data: any): Promise<void> {
	...
	private ["_" + IPCEvents.FETCH_CHANNEL](message: NodeMessage, data: any): Promise<void> | void {
	...
	private ["_" + IPCEvents.FETCH_GUILD](message: NodeMessage, data: any): Promise<void> | void {
	    if (!data.query || typeof data.query !== "string" || !/^[0-9]$/.test(data.query))
	...
	private async fetch(message: NodeMessage, data: any, op: IPCEvents): Promise<void> {
	    try {
	        if (data.clusterId) {
	...
	private async ["_" + IPCEvents.SERVICE_COMMAND](message: NodeMessage, data: any): Promise<void> {
	    try {
	        if (!data.serviceName) {
	            if (message.receptive)
	...
	private ["_" + IPCEvents.GET_STATS](message: NodeMessage): void{
	    message.reply({success: true, d: this.manager.stats});
	}
