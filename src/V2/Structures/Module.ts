...
export class Module{
    ...
    async checkGuildEnabled(guildID: string): Promise<boolean>{
        ...
        if(this.Hyperion.global.disabledModules.includes(this.name)){return false;}
        const config: GuildType | null = await this.Hyperion.managers.guild.getConfig(guildID);
        ...
        if(config.modules[this.name] === undefined){
            ...
            if(typeof config.modules[this.name] === "object"){
                if((config.modules[this.name] as {enabled: boolean}).enabled !== undefined){return (config.modules[this.name] as {enabled: boolean}).enabled;}
                return this.defaultStatus;
            }else{
    ...
    async diagnose(guild: Guild): Promise<EmbedResponse | null>{
        ...
        if(this.Hyperion.global.disabledModules.includes(this.name)){
