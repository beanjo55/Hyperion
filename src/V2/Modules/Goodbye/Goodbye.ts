...
class Goodbye extends Module{
    ...
    async getConfig(guild: string): Promise<GoodbyeConfig>{
        return await this.Hyperion.managers.guild.getModuleConfig<GoodbyeConfig>(guild, this.name);
    }
    ...
    async guildMemberRemove(...args: [Guild, Member]): Promise<void>{
        const guild = args[0];
        let member = args[1];
