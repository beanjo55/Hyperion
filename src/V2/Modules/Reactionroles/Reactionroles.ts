...
import {RRConfig, ReactionRole} from "../../Structures/MongoGuildManager";
...
class Reactionroles extends Module{
    ...
    async messageDelete(...args: [Message | {id: string; channel: GuildTextableChannel}]): Promise<void>{
        const msg = args[0];
        ...
        await this.Hyperion.managers.guild.updateModuleConfig(guild.id, this.name, config);
    }

    async messageReactionAdd(...args: [Message, Emoji, Member | {id: string}]): Promise<void>{
        const msg = args[0];
        const emote = args[1];
        const user = args[2].id;
        if(!(msg.channel.type === 0 || msg.channel.type === 5)){return;}
        const guild = msg.channel.guild;
        const enabled = await this.checkGuildEnabled(guild.id);
        if(!enabled){return;}
