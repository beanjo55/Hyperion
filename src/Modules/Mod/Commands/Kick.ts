...
class Kick extends Command{
    ...
    async execute(ctx: ICommandContext, Hyperion: IHyperion): Promise<string>{
        ...
        if(!bot.permission.has("kickMembers")){return " I need the `Kick Members` permission to well, kick members";}
        const toKick = await Hyperion.utils.strictResolver(ctx.args[0], ctx.guild.members);
        if(!toKick){return "I couldnt figure out what user that is";}
        if(toKick.id === ctx.user.id){return "You cant kick yourself";}
        if(toKick.id === Hyperion.client.user.id){return "I cant kick myself";}
        if(await ctx.module.isMod(Hyperion, toKick, ctx.guild)){return "That user is a mod and is protected from mod actions";}
        if(bot.roles.length === 0){return "I need a role higehr than the user's highest role to kick them, I cant do that with no roles";}
        ...
        if(userRoles[0] && userRoles[0].position >= botRoles[0].position){return "I cant kick someone with the same highest role or a higher role than me";}
