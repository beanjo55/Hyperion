...
class Role extends Command{
    ...
    async execute(ctx: ICommandContext, Hyperion: IHyperion): Promise<string>{
        ...
        if(bot.roles.length === 0){return "Due to Discord permissions, I need to have a role higher than a role to manage it. I cant do that with no roles.";}
        ...
        if(!user){return "I dont know who that us, try an user ID or mention.";}
        const botRoles = Hyperion.utils.sortRoles(bot.roles, ctx.guild.roles);
        if(botRoles[0].position <= roleObj.position){return "I cant manage roles that are my highest role, or above my highest role.";}
        if(operation === "add"){
