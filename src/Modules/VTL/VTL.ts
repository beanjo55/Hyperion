...
import {VTL as vtlconfig} from "../../Core/Managers/MongoGuildManager";
...
class VTL extends Module{
    ...
    async voiceChannelJoin(Hyperion: IHyperion, member: Member, channel: VoiceChannel): Promise<void>{
    ...
    async voiceChannelLeave(Hyperion: IHyperion, member: Member, channel: VoiceChannel): Promise<void>{
        const enabled = await this.checkGuildEnabled(channel.guild.id);
        if(!enabled){return;}
        const guild = channel.guild;
        const config = await this.getConfig(guild.id);
    ...
    async voiceChannelSwitch(Hyperion: IHyperion, member: Member, channel: VoiceChannel, oldChannel: VoiceChannel): Promise<void>{
        const enabled = await this.checkGuildEnabled(channel.guild.id);
        if(!enabled){return;}
        const guild = channel.guild;
        const config = await this.getConfig(guild.id);
